(function () {
  ...
  function initSilk(canvas) {
    ...
    var uColors = gl.getUniformLocation(program, "u_colors[0]");
    ...
    gl.uniform4f(uSurface, 2.4, 0.8, 0.02, 0.75);
    gl.uniform4f(uFinish, 6.28, 1.0, 0.04, 0.0);
    gl.uniform4f(uTransform, 1.0, 0.0, 0.4, 0.0);
    ...
    function render() {
      ...
      presence += (presenceTarget - presence) * 0.06;

      var elapsed = ((performance.now() - startTime) / 1000) * 0.18;

      gl.uniform4f(uScene, canvas.width, canvas.height, elapsed, 4.0);
      gl.uniform4f(uSpace, 0.0, 0.0, pointer.x, pointer.y);
  ...
  var instances = [];
  document.querySelectorAll("[data-silk-background]").forEach(function (canvas) {
    try {
      var instance = initSilk(canvas);
      if (instance) {
  ...
  // Der fest positionierte, dauerhaft animierte Canvas kann sich mit dem
  // Scroll-Snap-Übergang der Sektionen im Compositor überschneiden und dabei
  // kurz aufblitzen. Während des Scrollens pausieren vermeidet das.
  if (instances.length > 0) {
    var scrollStopTimer = null;
    var isPausedForScroll = false;
